...
- **The wheel works during a drag too.** The selection's anchor is an absolute transcript line, so scrolling under it extends the selection instead of dropping it. Only an idle highlight is cleared by the wheel. The same holds inside the input box, where the anchor is a row of your draft.
- **Box frames are not text.** A selection running through a tool box copies what is in the box, without its borders — and the borders never light up under the highlight either. The same goes for anything else Starline or Pi draws a frame around. A markdown table keeps every one of its pipes: a frame is only recognised as one when the run of rows is capped by a rule (`╭────╮`), which a table's `┌─┬─┐` and `├─┼─┤` borders are not. If you do want the box drawing itself, hold shift while dragging and use the terminal's own selection.
...
- Dragging selects the text, following the same `copyOnSelect` rule as the transcript. The rail, the borders and the metadata row are never part of a selection — neither highlighted nor copied.
- **A selection can cover a draft taller than the box.** Hold the drag past the text — on the frame, the metadata row, or up over the transcript — and the box scrolls, on the same timer the transcript uses, selecting the rows it brings in. A drag *along* the first or last row of the text is just a selection: unlike the transcript, the box has chrome on both sides, so there is a real "past the text" to wait for.
- **Scrolling the box never loses the selection.** It is anchored to the draft itself — a row of your text rather than a row of the screen — so the wheel, the caret and the auto-scroll all move the text underneath it and the anchor stays where you put it. What gets copied comes from the editor's own buffer, so rows that have scrolled out of the box come with it.
